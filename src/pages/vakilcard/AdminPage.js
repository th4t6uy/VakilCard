...
const card = "bg-white/70 dark:bg-white/[0.06] backdrop-blur-xl border border-slate-200/70 dark:border-white/10 shadow-sm rounded-2xl p-5";
...
const btnNeutral = btn + " bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 text-slate-700 dark:text-slate-300";
const btnGood = btn + " bg-emerald-50 dark:bg-emerald-500/10 border-emerald-200 dark:border-emerald-500/30 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300";
const btnWarn = btn + " bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/30 hover:bg-amber-100 dark:hover:bg-amber-500/20 text-amber-700 dark:text-amber-300";
const btnDanger = btn + " bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/30 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-700 dark:text-red-300";

function StatCard({ label, value, icon: Icon }) {
  return (
    <div className={card + " flex items-center gap-3"}>
      <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center flex-none">
...
function PlanBadge({ plan, status }) {
  ...
  return (
    <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-black uppercase tracking-wide ${pro ? "bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-300" : "bg-slate-100 dark:bg-white/[0.05] text-slate-600 dark:text-slate-300"}`}>
      {pro ? "Pro" : "Free"}
    </span>
  );
}
...
function TabBtn({ active, onClick, icon: Icon, children }) {
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 px-4 py-2 text-sm font-black border-b-2 -mb-px transition-colors ${active ? "border-indigo-600 text-indigo-700 dark:text-indigo-300" : "border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300"}`}
    >
...
const CARD_STATUS_STYLES = {
  Published: "bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300",
  Draft: "bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300",
  "Not Created": "bg-slate-100 dark:bg-white/[0.05] text-slate-500 dark:text-slate-400",
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={onClose}>
      <div className={card + " max-w-lg w-full max-h-[85vh] overflow-y-auto"} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div className="font-black text-slate-900 dark:text-white">{row.full_name || (row.username ? "@" + row.username : "User")}</div>
        ...
        <dl className="divide-y divide-slate-100 dark:divide-white/10">
          {fields.map(([k, v]) => (
            <div key={k} className="flex gap-4 py-2 text-sm">
              <dt className="w-40 flex-none text-slate-500 dark:text-slate-400 font-bold">{k}</dt>
              <dd className="text-slate-900 dark:text-white break-all">{v}</dd>
            </div>
          ))}
        </dl>
        {row.public_url && (
...
function UsersRegistry({ onError }) {
  ...
  const selectCls = "rounded-full border border-slate-200 dark:border-white/10 dark:bg-white/5 text-sm font-bold text-slate-700 dark:text-white px-3 py-2";
  ...
  return (
    <>
      <div className={card + " mb-4 flex flex-wrap items-center gap-3"}>
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-full border border-slate-200 dark:border-white/10 dark:bg-white/5 dark:text-white text-sm focus:outline-none focus:border-indigo-300 dark:focus:border-indigo-500/30"
          />
        </div>
      ...
      <div className="mb-3 text-xs font-bold text-slate-500 dark:text-slate-400">{total} registered user{total === 1 ? "" : "s"}</div>

      <div className={card + " p-0 overflow-hidden"}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm whitespace-nowrap">
            <thead>
              <tr className="text-left text-[11px] font-black uppercase tracking-wide text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-white/10">
              ...
              {rows.map((r) => (
                <tr key={r.account_id} className="border-b border-slate-100 dark:border-white/10 last:border-0 align-top">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-slate-500 dark:text-slate-400">{r.username ? "@" + r.username : "—"}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{fmtDate(r.registration_date)}</td>
                  <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{fmtDate(r.last_active)}</td>
                  <td className="px-4 py-3">
...
export default function AdminPage() {
  ...
  if (forbidden) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-white/[0.05] px-6">
        <div className={card + " max-w-sm text-center"}>
          <ShieldOff className="w-8 h-8 mx-auto mb-3 text-slate-400" />
          <div className="font-black text-slate-900 dark:text-white mb-1">Not authorized</div>
          <div className="text-sm text-slate-500 dark:text-slate-400">This account isn't on the VakilCard admin allowlist.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-white/[0.05]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex items-center gap-2.5 mb-6">
          <img src="/logo.png" alt="Vakilpedia" className="h-7 w-auto object-contain" />
          <BrandWordmark className="font-black text-slate-900 dark:text-white tracking-tighter text-lg" />
          <span className="rounded-full bg-slate-900 dark:bg-white dark:text-slate-900 text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 ml-1">VakilCard Admin</span>
        ...
        {error && <div className="mb-4 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-300 text-sm font-bold px-4 py-3">{error}</div>}
        ...
        <div className={card + " mb-4 flex flex-wrap items-center gap-3"}>
          ...
          <select
            ...
            className="rounded-full border border-slate-200 dark:border-white/10 dark:bg-white/5 text-sm font-bold text-slate-700 dark:text-white px-3 py-2"
          >
            <option value="ALL">All plans</option>
            <option value="FREE">Free</option>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-slate-100 dark:border-white/10 last:border-0 align-top">
                      <td className="px-4 py-3">
                        <div className="font-bold text-slate-900 dark:text-white">{r.full_name || "—"}</div>
                        <div className="text-slate-500 dark:text-slate-400">@{r.username}</div>
                        {!r.is_published && <span className="inline-block mt-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 text-[10px] font-black uppercase px-2 py-0.5">Draft / pending</span>}
                        {r.is_suspended && <span className="inline-block mt-1 ml-1 rounded-full bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-300 text-[10px] font-black uppercase px-2 py-0.5">Suspended</span>}
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                        <div>{r.phone || "—"}</div>
                      ...
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{new Date(r.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap justify-end gap-1.5">
